...
// Function to check for deadlock in a site and return coordinator ID
...
int main() {
    ...
    int site1Coordinator = checkDeadlockSite(processes, resources, 1);
    int site2Coordinator = checkDeadlockSite(processes, resources, 2);

    if (globalDeadlock) {
        printf("Deadlock detected in central coordinator\n");
    }
    if (site1Coordinator != -1) {
        printf("Deadlock detected in site 1, local coordinator: %d\n", site1Coordinator);
    }
    if (site2Coordinator != -1) {
        printf("Deadlock detected in site 2, local coordinator: %d\n", site2Coordinator);
    }
    if (!globalDeadlock && site1Coordinator == -1 && site2Coordinator == -1) {
